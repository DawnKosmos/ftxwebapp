...
func (o *StopOrder) Evaluate(w Communicator, f exchange.Exchange) error {
...
func (p *StopPrice) Evaluate(ws Communicator, f exchange.Exchange, sides string, ticker string, size float64) (err error) {
	var side bool
	factor := 1.0
	if sides == "buy" {
		side = true
