...
func (p *Price) Evaluate(f exchange.Exchange, ws Communicator, side string, ticker string, size float64) (err error) {
	var mp float64
...
func (p *Price) EvaluatePrice(f exchange.Exchange, sides string, ticker string, size float64, mp float64, ws Communicator, ro bool) error {
	var side bool
	if sides == "buy" {
		side = true
	}
...
func (p *Price) EvaluateDifference(f exchange.Exchange, side string, ticker string, size float64, mp float64, ws Communicator, ro bool) error {
	factor := 1.0
	ss := true
	if side == "sell" {
		factor = -1.0
...
func (p *Price) EvaluatePercentual(f exchange.Exchange, side string, ticker string, size float64, mp float64, ws Communicator, ro bool) error {
	factor := 1.0
	ss := true
	if side == "sell" {
		factor = -1.0
